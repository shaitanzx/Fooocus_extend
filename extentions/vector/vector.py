...
def single_image(single_upload):
    if len(single_upload) == 1:
        ...
        return gr.update (value=single_upload[0],visible=True),gr.update(visible=False)
    else:
    ...
    with gr.Row():
        with gr.Column():
            with gr.Row():
                ...
                image_single=gr.Image(label="Reference image",visible=False,height=260,interactive=False,type="filepath")
            with gr.Row():

...
def define_D(
    ...
    netD = MultiscaleDiscriminator(
        input_nc, opt, ndf, n_layers_D, norm_layer, use_sigmoid, num_D, getIntermFeat
    )
    print(netD)
    if len(gpu_ids) > 0:
        assert torch.cuda.is_available()
    ...
    def __init__(
        ...
        for i in range(num_D):
            netD = NLayerDiscriminator(
                input_nc, opt, ndf, n_layers, norm_layer, use_sigmoid, getIntermFeat
            )
            if getIntermFeat:
...
class NLayerDiscriminator(nn.Module):
    def __init__(
        ...
        sequence += [
            [SN(nn.Conv2d(nf, 1, kernel_size=kw, stride=1, padding=padw), opt.use_SN)]
        ]
...
class Patch_Attention_4(nn.Module):  ## While combine the feature map, use conv and mask
    ...
    def forward(self, z, mask):  ## The shape of mask is Batch*1*H*W
        ...
        correlation_matrix = correlation_matrix.masked_fill(
            non_mask_region == 1.0, -1e9
        )
        correlation_matrix = F.softmax(correlation_matrix, dim=2)

        _, max_arg = torch.max(correlation_matrix, dim=2)

        composed_unfold = self.Hard_Compose(x_unfold, 2, max_arg)
        composed_fold = F.fold(
            composed_unfold,
            output_size=(h, w),
            kernel_size=(self.patch_size, self.patch_size),
            padding=0,

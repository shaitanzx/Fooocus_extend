...
class Pix2PixHDModel(BaseModel):
    ...
    def init_loss_filter(self, use_gan_feat_loss, use_vgg_loss, use_smooth_L1):
        ...
        def loss_filter(g_gan, g_gan_feat, g_vgg, g_kl, d_real, d_fake, smooth_l1):
            return [
                l
                for (l, f) in zip(
                    (g_gan, g_gan_feat, g_vgg, g_kl, d_real, d_fake, smooth_l1), flags
                )
                if f
            ]
    ...
    def initialize(self, opt):
        ...
        self.gen_features = (
            self.use_features and not self.opt.load_features
        )  ## it is also false
        input_nc = (
            opt.label_nc if opt.label_nc != 0 else opt.input_nc
        )  ## Just is the origin input channel #
        ...
        if self.isTrain:
            ...
            self.loss_filter = self.init_loss_filter(
                not opt.no_ganFeat_loss, not opt.no_vgg_loss, opt.Smooth_L1
            )
            ...
            self.loss_names = self.loss_filter(
                "G_GAN", "G_GAN_Feat", "G_VGG", "G_KL", "D_real", "D_fake", "Smooth_L1"
            )
        ...
        for i in np.unique(inst_np):
            ...
            if label in features_clustered:
                ...
                for k in range(self.opt.feat_num):
                    feat_map[idx[:, 0], idx[:, 1] + k, idx[:, 2], idx[:, 3]] = feat[
                        cluster_idx, k
                    ]
        if self.opt.data_type == 16:

...
#        tempmodel = random.choice(modellist)
    ...
    currentlyselectedmodel = base_model_foocus
#    if(onlyupscale==True):
...
#            print("model reloaded")
    ...
    if(onlyupscale==False):  # only do txt2img when onlyupscale is False
        ...
        if(autonegativeprompt):
            negativeprompt = build_dynamic_negative(positive_prompt=randomprompt, insanitylevel=autonegativepromptstrength,enhance=autonegativepromptenhance, existing_negative_prompt=originalnegativeprompt, base_model_obp=base_model_obp)
        ...
        print ('aaaaaaaaaaaaaaaaaaaaa',filename)
        ...
        print('sssssssssssssssss',model)
            # set the model here
            #if(originalmodel!="currently selected model"):
        option_payload = {
            "sd_model_checkpoint": model
                }
        print('sssssssssssssssss',option_payload)

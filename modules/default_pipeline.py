...
def process_diffusion(positive_cond, negative_cond, steps, switch, width, height, image_seed, callback, sampler_name, 
    ...
    def conditioning_modifier(model, x, timestep, uncond, cond, cond_scale, model_options, seed):
        try:
            # 1. Определяем шаг СРАЗУ (до любой логики, чтобы не потерять его при ошибке)
            try:
                cs = int(timestep[0].item())
            except:
                cs = 0
                
            # 2. Сразу отмечаем шаг в callback (гарантия, что set не останется пустым)
            callback.lbw_steps.add(cs)
            callback.lbw_info[cs] = "WAITING"

            # 3. Безопасный доступ к переменным замыкания
            loras = target_unet.model_options.get("_lbw_loaded_loras", []) or []
            sigmas = minmax_sigmas  # берём из замыкания
            ...
            def lora_step(step_idx):
                ...
                for lora in loras:
                    ...
                    if step_idx >= start and (stop is None or step_idx < stop):
                        out.append(lora.get('lora_name'))
                return out

            current_lora = lora_step(cs)
            prev_lora = lora_step(cs - 1) if cs > 0 else []

            # 4. Финальная запись логов
            callback.lbw_info[cs] = f"LoRA cur: {current_lora} | prev: {prev_lora}"
        except Exception as e:
            # Если где-то внутри ошибка, записываем её, но не роняем хук
            try:

...
def generate_temp_filename(folder='./outputs/', extension='png',name_prefix=''):
    ...
    filename = f"{name_prefix}{time_string}_{random_number}.{extension}"
...
def apply_lbw_patches(patcher, patch_dict: dict, base_weight: float, lbw_preset: str, slot_map: dict, lbwe_preset: str = None):
    print(f"\033[93m[DEBUG] apply_lbw_patches called, patches count: {len(lora_patches)}\033[0m")

    ...
    for f_name in batch_files:
        ...
        if image_action == 'Upscale': 
            ...
            if ratio_scale=="to OUTPUT":
                ...
                w = int(width * ratio_scale)
                h = int(height * ratio_scale)
...
with shared.gradio_root:
    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            adetail_tab.select(lambda: 'adetail', outputs=current_tab, queue=False, _js=down_js, show_progress=False)
              ...
              with gr.Accordion('Extention', open=False):
                ...
                with gr.Accordion('modules', open=False,elem_classes="nested-accordion"):
                  ...
                  with gr.Tab(label='Cleaner', id='clean_tab') as clean_tab:
                        with gr.Tab(label='Image'):
                            with gr.Row():
                                with gr.Column():
                                    ...
                                    clean_preview=gr.Image(label="Process preview",visible=False,height=260,interactive=False,type='pil')
                            with gr.Row():
                        ...
                        clean_button.click(lambda: (gr.update(interactive=False),gr.update(visible=True),gr.update(visible=False),gr.update(visible=False)),outputs=[clean_button,progress_image,clean_zip,init_img_with_mask]) \
                            .then(fn=cleaner.clean_object_init_img_with_mask,inputs=[image_files,init_img_with_mask,image_mask_check,image_mask_load],outputs=[progress_image,result_gallery,result_gallery,clean_preview]) \
                            .then(fn=cleaner.clean_zip,inputs=[result_gallery],outputs=[clean_zip]) \
                            .then(lambda: (gr.update(interactive=True),gr.update(visible=True),gr.update(visible=True),gr.update(visible=False)),outputs=[clean_button,clean_zip,init_img_with_mask,clean_preview])
                        ...
                        clean_button_video.click(lambda: (gr.update(visible=True),gr.update(interactive=False)),outputs=[progress_video,clean_button_video]) \
                            .then(cleaner.video_clean_process,inputs=[video_files,clean_frame,mask_check,mask_load],outputs=[progress_video]) \
                            .then(lambda: (gr.update(interactive=True)),outputs=[clean_button_video])
        ...
        with gr.Column(scale=1, visible=modules.config.default_advanced_checkbox) as advanced_column:
            ...
            with gr.Tab(label='Advanced'):
                ...
                with gr.Column(visible=modules.config.default_developer_debug_mode_checkbox) as dev_tools:
                    with gr.Tab(label='Debug Tools'):
                        ...
                        rescale_cfg = gr.Slider(label='CFG rescale', minimum=0, maximum=1, step=0.01,
                                                 value=modules.config.default_rescale_cfg,visible=False,interactive=True)
                ...
                def update_ad_model_choices():
                    models=[gr.update(choices = modules.config.yolo_filenames) for _ in range(modules.config.default_adetail_tab)]
        ...
        load_data_outputs = [advanced_checkbox, image_number, prompt, negative_prompt, style_selections,
                             ...
                             adm_scaler_negative, adm_scaler_end, refiner_swap_method, adaptive_cfg, rescale_cfg, type_cfg, clip_skip,
